...
describe('SocialCards', () => {
	...
	test('SocialCards snapshot renders', () => {
		const cSocialCards = renderer.create(<SocialCards props={myState} />);
		let tree = cSocialCards.toJSON();
	...
	test('SocialCard snapshot renders', () => {
		...
		let tree = cSocialCard.toJSON();
		expect(tree).toMatchSnapshot();
	});

	test('SocialCards gatherData function is called properly', () => {
		const cSocialCards = mount(<SocialCards props={myState} />);
